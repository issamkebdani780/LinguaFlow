...
const Dashboard = ({ onLogout }) => {
  ...
  const [words, setWords] = useState([
    { id: 1, word: 'Serendipity', arabic: 'صدفة سعيدة', date: '2025-12-20' },
    { id: 2, word: 'Ephemeral', arabic: 'زائل', date: '2025-12-19' },
    { id: 3, word: 'Resilience', arabic: 'مرونة', date: '2025-12-18' }
  ]);
  const [searchQuery, setSearchQuery] = useState('');
  
  // Chatbot States
  const [chatMessages, setChatMessages] = useState([
    { id: 1, type: 'bot', message: 'Hi! I\'m your LinguaFlow AI assistant. Send me any English word and I\'ll help you learn it!' }
  ]);
  ...
  const handleAddWord = (e) => {
    e.preventDefault();
    if (newWord.trim() && arabicTranslation.trim()) {
      const word = {
        id: Date.now(),
        word: newWord,
  ...
  const handleSendMessage = async (e) => {
    ...
    try {
      ...
      const botMessage = {
        id: Date.now() + 1,
        type: 'bot',
        message: data.message || data.response || 'Message received! Processing your request...'
      };
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-white flex">
      {/* Sidebar */}
      <aside className={`fixed lg:static inset-y-0 left-0 z-50 w-64 bg-[#0B0C10] border-r border-white/5 transform transition-transform duration-300 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`}>
        <div className="flex flex-col h-full">
          ...
          <div className="p-6 border-b border-white/5">
            <div className="flex items-center gap-2">
              ...
              <span className="font-bold text-lg">Lingua<span className="text-sky-400">Flow</span></span>
            </div>
          </div>
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-gray-400 mt-1">
                  ...
                  {activeTab === 'my-words' && `${words.length} words in your collection`}
              ...
              <div className="bg-gradient-to-b from-[#1A1D24]/60 to-[#111318]/80 backdrop-blur-md border border-white/10 rounded-3xl p-8">
                ...
                <div className="mt-6 p-4 bg-sky-500/10 border border-sky-500/20 rounded-xl">
                  <p className="text-sm text-sky-400">
                    💡 <strong>Tip:</strong> Words added here will be automatically reviewed using spaced repetition.
                  </p>
                </div>
              </div>
            </div>
              ...
              <div className="space-y-3">
                {filteredWords.length === 0 ? (
                  <div className="text-center py-12 bg-gradient-to-b from-[#1A1D24]/60 to-[#111318]/80 backdrop-blur-md border border-white/10 rounded-3xl">
                    ...
                    <h3 className="text-xl font-semibold text-gray-400 mb-2">No words found</h3>
                    <p className="text-gray-500">Start adding words to build your vocabulary!</p>
                  </div>
                ) : (
                    ...
                    >
                      <div className="flex items-start justify-between">
                        <div className="flex-1">
                          ...
                          <p className="text-lg text-gray-400 mb-2" dir="rtl">{word.arabic}</p>
                          <p className="text-xs text-gray-500">Added: {word.date}</p>
                        </div>
                        <div className="flex items-center gap-2">
